...
A policy document can be traversed and manipulated via:
...
>>> encode $ statement (allow & sid ?~ "cd3ad3d9-2776-4ef1-a904-4c229d1642ee")
foo
...
See the 'allow' and 'deny' smart constructors to create a 'Statement'.

Example:

>>> encode $ statement allow
foo

>>> encode $ statement deny
bar
...
>>> encode $ statement (allow & principal ?~ Match Everyone)
foo

>>> encode $ statement (allow & principal ?~ Match (AWS (pure "foo")))
bar

>>> encode $ statement (allow & principal ?~ Not (CanonicalUser "1"))
baz
...
>>> encode $ statement (allow & action .~ Match ["foo", "bar"])
foo

>>> encode $ statement (allow & action .~ Not ["baz"])
baz

>>> encode $ statement (allow & action .~ wildcard)
baz
...
>>> encode $ statement (allow & resource ?~ Match ["foo", "bar"])
foo

>>> encode $ statement (allow & resource ?~ Not ["baz"])
baz

>>> encode $ statement (allow & resource ?~ wildcard)
baz
...
-- | Match anything using the wildcard @"*"@.
...
>>> encode $ statement (condition & sid ?~ "1")
foo
